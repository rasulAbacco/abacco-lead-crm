 ...
 */
router.post("/:id/activate", authenticate, authorizeRole("ADMIN"), async (req, res) => {
    try {
        const id = Number(req.params.id);
 ...
 */
router.post("/:id/deactivate", authenticate, authorizeRole("ADMIN"), async (req, res) => {
    try {
        const id = Number(req.params.id);
 ...
 */
router.post("/:id/rules", authenticate, authorizeRole("ADMIN"), async (req, res) => {
    try {
        const id = Number(req.params.id);
        if (isNaN(id)) return res.status(400).json({ message: "Invalid id" });
 ...
 */
router.put("/rules/:ruleId", authenticate, authorizeRole("ADMIN"), async (req, res) => {
    try {
        const ruleId = Number(req.params.ruleId);
        if (isNaN(ruleId)) return res.status(400).json({ message: "Invalid rule id" });
        ...
        const updated = await prisma.incentiveRule.update({
            ...
            data: {
                ...
                leadsRequired: leadsRequired !== undefined ? Number(leadsRequired) : rule.leadsRequired,
 ...
 */
router.delete("/rules/:ruleId", authenticate, authorizeRole("ADMIN"), async (req, res) => {
    try {
        const ruleId = Number(req.params.ruleId);
        if (isNaN(ruleId)) return res.status(400).json({ message: "Invalid rule id" });
    ...
    try {
        ...
        for (const lead of todayLeads) {
            ...
            if (!planList || planList.length === 0) {
                if (!groups[unknownPlanKey]) groups[unknownPlanKey] = { plan: null, leads: [] };
        ...
        return res.status(500).json({ error: "Failed to compute incentive progress" });
    }
});
