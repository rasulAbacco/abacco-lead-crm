...
/**
 * 🔹 NEW: Edit / Update an allowed IP's address and/or label
 */
export const editAllowedIP = async (req, res) => {
  try {
    const { id } = req.params;
    const { ipAddress, label } = req.body;
    ...
    const updated = await updateAllowedIP(id, { ipAddress, label });
    ...
    // Handle duplicate IP (unique constraint on ipAddress)
    if (error.code === "P2002") {
      return res.status(400).json({
        success: false,
        message: "IP already exists",
