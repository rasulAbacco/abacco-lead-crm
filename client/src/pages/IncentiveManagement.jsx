                  ...
                  <tr key={plan.id} className="hover:bg-purple-50 transition">
                    ...
                    <td className="px-6 py-4 hidden md:table-cell">
                      <div className="space-y-2 text-sm">
                            ...
                            <ul className="ml-3 space-y-1">
                              {groups.Attendees.map((r) => (
                                <li key={r.id} className="flex justify-between">
                                  <span>
                                    {r.country ? `${r.country} - ` : ""} Min
                                    Count: {r.attendeesMinCount} →{" "}
                                    {r.leadsRequired} leads
                                  </span>
                        ...
                        {/* Member Attendees */}
                        {groups["Member Attendees"].length > 0 && (
                          <div>
                            <p className="font-bold text-indigo-700">
                              Member Attendees
                            </p>
                            <ul className="ml-3 space-y-1">
                              {groups["Member Attendees"].map((r) => (
                                <li key={r.id} className="flex justify-between">
                                  <span>
                                    {r.country ? `${r.country} - ` : ""}
                                    {r.attendeesMinCount
                                      ? `Min Count: ${r.attendeesMinCount} → `
                                      : ""}
                                    {r.leadsRequired} leads
                                  </span>
