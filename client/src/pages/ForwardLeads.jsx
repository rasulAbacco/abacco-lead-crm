...
const ForwardLeads = () => {
    ...
    try {
      ...
      setEmployees(
        data.map((emp) => {
          ...
          if (debugMode) {
            console.log(`Employee ${emp.fullName} has ${emp.leads.length} leads`);
            emp.leads.forEach((lead, index) => {
              ...
              console.log(`Lead ${index}: date=${lead.date}, formatted=${leadDateStr}, forwarded=${lead.forwarded}, qualified=${lead.qualified}`);
            });
          }
          ...
          const oldUnforwardedLeads = emp.leads.filter((lead) => {
            ...
            const isQualifiedOrNull = lead.qualified === true || lead.qualified === null;
          ...
          if (debugMode) {
            console.log(`Filtered results for ${emp.fullName}: today=${todayLeads.length}, old=${oldUnforwardedLeads.length}`);
          }
  ...
  const forwardLead = async (leadId, empId, isOldLead = false) => {
    ...
    try {
      const res = await fetch(`${API_BASE_URL}/api/employees/leads/${leadId}/forward`, {
        method: "POST",
      });
      ...
      setEmployees((prevEmployees) =>
        prevEmployees.map((emp) => {
          ...
          if (isOldLead) {
            ...
            // Update count
            updatedEmp.oldUnforwardedLeads = emp.oldLeads.filter(l => l.id !== leadId && !l.forwarded).length;
          } else {
  ...
  const markQualified = async (leadId, empId, isQualified, isOldLead = false) => {
    setActionLoading(prev => ({ ...prev, [leadId]: 'qualifying' }));
    try {
      const res = await fetch(`${API_BASE_URL}/api/employees/leads/${leadId}/qualify`, {
        method: "POST",
        ...
        prevEmployees.map((emp) => {
          ...
          if (isOldLead) {
            ...
            if (isQualified === false) {
              updatedEmp.oldLeads = updatedEmp.oldLeads.filter(l => l.id !== leadId);
  ...
  const viewLeadDetails = (emp, isOldLead = false) => {
    setSelectedEmployee(
      selectedEmployee?.id === emp.id && selectedEmployee?.isOldLead === isOldLead
        ? null
        : { ...emp, isOldLead }
    );
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-blue-50 to-indigo-50 p-8">
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="flex justify-between">
          ...
          >
            {debugMode ? <Bug className="w-4 h-4" /> : <Eye className="w-4 h-4" />}
          ...
          <div className="bg-gradient-to-br from-purple-50 to-indigo-50 rounded-2xl shadow-xl border border-purple-200 overflow-hidden">
            ...
            <div className="flex border-b border-purple-200">
              {[
                ...
                { id: "filtering", label: "Filtering", icon: <Filter className="w-4 h-4" /> },
              ].map((tab) => (
                <button
                  key={tab.id}
            ...
            <div className="p-6">
              {debugTab === "overview" && (
                <div className="space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div className="bg-white rounded-lg p-4 shadow-sm border border-purple-100">
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <h4 className="font-semibold text-gray-800">Today's Date (USA)</h4>
                      </div>
                      <p className="text-lg font-mono text-purple-600">
                        {format(toZonedTime(new Date(), USA_TZ), "yyyy-MM-dd", { timeZone: USA_TZ })}
                      </p>
                    </div>
                    ...
                    <div className="bg-white rounded-lg p-4 shadow-sm border border-purple-100">
                      ...
                      <p className="text-lg font-mono text-purple-600">{employees.length}</p>
                    </div>
                    
                    <div className="bg-white rounded-lg p-4 shadow-sm border border-purple-100">
                      <div className="flex items-center gap-2 mb-2">
                        <Clock className="w-4 h-4 text-purple-600" />
                        <h4 className="font-semibold text-gray-800">Employees with Old Leads</h4>
                      </div>
                      <p className="text-lg font-mono text-purple-600">
                  ...
                  <div className="bg-white rounded-lg p-4 shadow-sm border border-purple-100">
                    <h4 className="font-semibold text-gray-800 mb-3">Employee Lead Summary</h4>
                    <div className="overflow-x-auto">
                      <table className="w-full text-sm">
                        <thead>
                          <tr className="border-b border-purple-100">
                            <th className="text-left py-2 px-3 font-medium text-gray-700">Employee</th>
                            <th className="text-center py-2 px-3 font-medium text-gray-700">Today</th>
                            <th className="text-center py-2 px-3 font-medium text-gray-700">Old</th>
                            <th className="text-center py-2 px-3 font-medium text-gray-700">Total</th>
                          </tr>
                        </thead>
                        <tbody>
                          {employees.map(emp => (
                            <tr key={emp.id} className="border-b border-purple-50">
              ...
              {debugTab === "filtering" && (
                <div className="space-y-4">
                  <div className="bg-white rounded-lg p-4 shadow-sm border border-purple-100">
                    <h4 className="font-semibold text-gray-800 mb-3">Filtering Logic</h4>
                    <div className="space-y-3 text-sm">
                      <div className="flex items-start gap-2">
                        <div className="w-6 h-6 rounded-full bg-blue-100 text-blue-700 flex items-center justify-center text-xs font-bold mt-0.5">1</div>
                        <div>
                          <p className="font-medium text-gray-700">Today's Leads</p>
                          <p className="text-gray-600">Leads where date matches today's date in USA timezone</p>
                        </div>
                      </div>
                      <div className="flex items-start gap-2">
                        <div className="w-6 h-6 rounded-full bg-amber-100 text-amber-700 flex items-center justify-center text-xs font-bold mt-0.5">2</div>
                        <div>
                          <p className="font-medium text-gray-700">Old Unforwarded Leads</p>
                  ...
                  <div className="bg-white rounded-lg p-4 shadow-sm border border-purple-100">
                    <h4 className="font-semibold text-gray-800 mb-3">Sample Lead Data</h4>
                    <div className="space-y-3">
                      {employees.slice(0, 2).map(emp => (
                        <div key={emp.id} className="border border-purple-100 rounded-lg p-3">
                          <h5 className="font-medium text-gray-800 mb-2">{emp.name}</h5>
                          {emp.leads.slice(0, 1).map(lead => (
                            <div key={lead.id} className="text-xs space-y-1">
                              ...
                              <div className="flex justify-between">
                                ...
                                <span className="font-mono">{lead.contactDate}</span>
                              </div>
                              <div className="flex justify-between">
                                <span className="text-gray-500">Forwarded:</span>
                                <span className={`font-semibold ${lead.forwarded ? 'text-green-600' : 'text-red-600'}`}>
                              ...
                              <div className="flex justify-between">
                                ...
                                <span className={`font-semibold ${
                                  lead.qualified === true ? 'text-green-600' : 
                                  lead.qualified === false ? 'text-red-600' : 'text-gray-600'
                                }`}>
                                  {lead.qualified === true ? 'Yes' : 
                                   lead.qualified === false ? 'No' : 'Not Set'}
                                </span>
                              </div>
                            </div>
                          ))}
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
          <div className="p-6 border-b border-gray-200 flex items-center justify-between">
            <h2 className="text-xl font-bold text-gray-900">Today's Employee Leads</h2>
            <span className="text-sm text-gray-500">
              Showing {employees.filter(e => e.dailyLeads > 0).length} employees with leads
            </span>
          </div>
                      ...
                      <React.Fragment key={`today-${emp.id}`}>
                        <tr className="hover:bg-gray-50 transition-colors">
                          ...
                          <td className="px-6 py-4 text-center">
                            ...
                            >
                              {selectedEmployee?.id === emp.id && !selectedEmployee?.isOldLead ? "Hide Details" : "View Details"}
                            </button>
                          </td>
                        </tr>

                        {/* Lead Details Expanded View */}
                        {selectedEmployee?.id === emp.id && !selectedEmployee?.isOldLead && emp.leads.length > 0 && (
                          <tr>
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
          <div className="p-6 border-b border-gray-200 flex items-center justify-between">
            <h2 className="text-xl font-bold text-gray-900">Previous Unforwarded Leads</h2>
            <span className="text-sm text-gray-500">
              Showing {employees.filter(e => e.oldUnforwardedLeads > 0).length} employees with unforwarded leads
            </span>
          </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              <tbody className="divide-y divide-gray-200">
                {employees.filter(emp => emp.oldUnforwardedLeads > 0).length === 0 ? (
                  <tr>
                    <td colSpan="4" className="px-6 py-12 text-center text-gray-500">
                  ...
                  employees
                    ...
                    .map((emp, index) => (
                      <React.Fragment key={`old-${emp.id}`}>
                        <tr className="hover:bg-gray-50 transition-colors">
                          ...
                          <td className="px-6 py-4 whitespace-nowrap">
                            <div className="flex items-center gap-2">
                              ...
                              <span className="text-gray-900 font-semibold">{emp.oldUnforwardedLeads}</span>
                              <span className="text-gray-500 text-sm">leads</span>
                            </div>
                          </td>
                          <td className="px-6 py-4 text-center">
                        ...
                        {/* Lead Details Expanded View */}
                        {selectedEmployee?.id === emp.id && selectedEmployee?.isOldLead && emp.oldLeads.length > 0 && (
                          <tr>
                            <td colSpan="4" className="bg-gradient-to-r from-amber-50 to-orange-50">
                              <div className="p-6 space-y-4">
                                <h3 className="text-lg font-bold text-gray-900 mb-4">
...
// Lead Card Component
const LeadCard = ({ lead, empId, forwardLead, markQualified, actionLoading, isOldLead }) => (
  <div
  ...
  >
    ...
    <div className="border-t border-gray-200 pt-4 flex flex-wrap gap-3">
      ...
      <button
        ...
        disabled={lead.qualified === true || actionLoading[lead.id] === 'qualifying'}
      ...
      <button
        ...
        disabled={lead.qualified === false || actionLoading[lead.id] === 'qualifying'}
        className={`px-4 py-2 rounded-lg font-medium text-sm flex items-center gap-2 transition-all ${
      ...
      <button
        ...
        title={
          lead.qualified === false 
            ? "Cannot forward disqualified leads" 
            : ""
        }
      >
        {actionLoading[lead.id] === 'forwarding' ? (
...
const TextBlock = ({ icon, label, value }) => (
  <div className="flex items-start gap-2 text-sm">
    ...
    <div className="flex-1">
      ...
      <p className="text-gray-900 font-medium whitespace-pre-wrap">{value || "Not Available"}</p>
    </div>
  </div>
);
