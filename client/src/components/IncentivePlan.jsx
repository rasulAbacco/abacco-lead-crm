...
const groupRules = (rules = []) => {
  return {
    ...
    "Member Attendees": rules.filter((r) => r.leadType === "Member Attendees"),
  ...
  return (
    ...
    >
      ...
      <div className="px-6 pb-6 flex-1 space-y-6">
        ...
        {/* Member Attendees */}
        {ruleGroups["Member Attendees"].length > 0 && (
          <div>
            <h4 className="text-sm font-bold text-indigo-600 mb-2">
              Member Attendees Rules
            </h4>
            <div className="space-y-2">
              {ruleGroups["Member Attendees"].map((r, idx) => (
                <RuleRow key={idx} rule={r} />
              ))}
            </div>
          </div>
  ...
  const description =
    rule.leadType === "Attendees"
      ...
      : rule.leadType === "Member Attendees"
        ? `${rule.country}${rule.attendeesMinCount ? ` • Min Members ${rule.attendeesMinCount}` : ""}`
      ...
      <div className="flex items-center space-x-3">
        ...
        <div className="text-sm">
          ...
          <p className="text-xs text-gray-500">
            {rule.leadsRequired} qualified leads
          </p>
        </div>
      </div>
      ...
      <div className="mx-auto">
        <div className="mb-10 text-center md:text-left">
          ...
          <p className="mt-2 text-gray-700 text-lg">
            Achieve your targets and unlock rewards based on daily qualified
            leads.
          </p>
        </div>
