...
export default function MyLeadsTable() {
    ...
    const fetchLeads = async () => {
        setLoading(true);
        try {
            const res = await fetch(`${API_BASE_URL}/api/employees/leads/${employeeId}`);
                                        ...
                                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                                            {isEditing ? (
                                                <>
                                                    ...
                                                    <EditableField label="Lead Email" icon={Mail} color="pink" value={editForm.leadEmail} onChange={(v) => setEditForm({ ...editForm, leadEmail: v })} />
                                                    <EditableField label="CC Email" icon={MailPlus} color="violet" value={editForm.ccEmail} onChange={(v) => setEditForm({ ...editForm, ccEmail: v })} colSpan="sm:col-span-2 lg:col-span-1" />
                                                    <EditableField label="Link" icon={ExternalLink} color="indigo" value={editForm.link} onChange={(v) => setEditForm({ ...editForm, link: v })} colSpan="sm:col-span-2 lg:col-span-2" />
                                                </>
                                            ) : (
                                                <>
                                                    <InfoCard icon={Building2} color="emerald" title="Agent" value={safe(lead.agentName)} />
                                                    <InfoCard icon={Calendar} color="purple" title="Date" value={formatDate(lead.date)} />
                                                    <InfoCard icon={Globe} color="teal" title="Country" value={safe(lead.country)} />
                                                    ...
                                                    <InfoCard icon={Mail} color="orange" title="Client Email" value={safe(lead.clientEmail)} />
                                                    <InfoCard icon={Mail} color="pink" title="Lead Email" value={safe(lead.leadEmail)} />
                                                    <div className="flex items-start gap-3 sm:col-span-2 lg:col-span-1">
...
const ExpandedContent = ({ lead, isEditing, editForm, setEditForm }) => (
    <div className="mt-6 pt-6 border-t border-slate-200 space-y-4">
        {isEditing ? (
            <>
                <div>
                    <label className="block text-sm font-medium text-slate-700 mb-2">Email Pitch</label>
                    <textarea
                        value={editForm.emailPitch}
                        onChange={(e) => setEditForm({ ...editForm, emailPitch: e.target.value })}
                        rows={6}
                        className="w-full px-4 py-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none text-sm"
                    />
                </div>
                <div>
                    <label className="block text-sm font-medium text-slate-700 mb-2">Email Response</label>
                    <textarea
                        value={editForm.emailResponce}
                        onChange={(e) => setEditForm({ ...editForm, emailResponce: e.target.value })}
                        rows={6}
                        className="w-full px-4 py-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none text-sm"
                    />
                </div>
            ...
            <>
                {lead.emailPitch ? (
                    <EmailSection title="Email Pitch" color="blue" icon={Send} text={lead.emailPitch} />
                ) : (
